...
.option{ display:none; color:'#000'; cursor:pointer; padding:10px; }
.slider-block{ background: none; color: #000; }
.slider-block p{font-weight: 400;}
.topmain-slider .flexslider .slides img { opacity: .4; height: 110vh; }
...
.row.cstm-bnner ul{ display: flex; position: relative; left: 55px; top: 30px; }
input.search_input_banner {
    border-top-left-radius: 30px; border-bottom-left-radius: 30px; border-right: 1px solid #c7c7c7;
}

input#zipcode{
    border-top-right-radius: 30px; border-bottom-right-radius: 30px; border-left: 1px solid #c7c7c7;
}
.row.cstm-bnner li { margin: 0; width: 33.33%; }
.slider-block button{ border-radius: 50px; padding: 6px 13px; font-size: 13px; }
.slider-block li button{ padding: 6px 20px; font-size: 15px; }
.cstm-bnner ul li:last-child{ position: relative; right: 104px; top: 5.2px; }
</style>
<section class="main-slider topmain-slider">
    ...
    <div class="slider-block">
        <h1 style="word-spacing: 10px";>FIND SPORTS, WELLNESS & ADVENTUROUS ACTIVITIES<br/></h1>
...
Participate in activities at local business locations, online, or while traveling.</p>
		...
		<form id="searchform" method="get" action="/search-result-location">
			...
			<div class="row cstm-bnner">
				<div class="col-sm-12">
					...
					<ul>
                    	<li>
							<input autocomplete="off" type="text" name="label" id="bsearch_activity_label" placeholder="Search by activity or business" value="@if(isset($selected_label) && $selected_label != NULL){{$selected_label }}@endif"  class="search_input_banner" />
	...
	<script>
		$(document).ready(function(){ 
			$("#bsearch_activity_label").keyup(function(){ alert('call'); return false;
				...
				$.ajax({
					...
					success: function(data){ 
						...
						$("#bsearch_activity_label").css("background","#FFF");
					}
				});
			});
			$(document).on('click', '.searchclickactivity', function(){
				$("#bsearch_activity_label").val($(this).attr('data-num'));
		...
		<div class="container">
			<div class="row">
				<div class="col-md-8 mysrchloc">
					<?php /*?><div class="card-claimed-business">
    					<div class="img-claimed-business">
