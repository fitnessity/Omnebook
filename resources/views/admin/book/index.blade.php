...
@section('content')
    ...
    <p>
        <a href="{!! route('create-new-bookactivity') !!}" class="btn btn-success">Add New</a>
    </p>
    ...
    <div class="panel panel-default">
        ...
        {!! Form::open(array('route' => 'delete-plans', 'id' => 'planlist')) !!}
            ...
            <div class="table-responsive">
              <table id="online_list" class="table table-bordered table-striped {{ count($bookactivity) > 0 ? 'datatable' : '' }} table-hover ">
                ...
                <tbody>
                    @if (count($bookactivity) > 0)
                    <?php $i = 0 ?>
                        @foreach ($bookactivity as $value)
                          <?php $i++ ?>
                            <tr id="item-{!! $value->id !!}">
                               <!--  <td><input type="checkbox" name="onlineIds[]" value="{{$value->id}}"></td> -->
                                <td><img src="{{ asset('public/uploads/book/thumb/'.$value->image) }}" width="150" height="auto" /></td>
                                <td>{{ $value->title}}</td>
                              
                                <td>
                                  <a href="\admin\book\edit\{{$value->id}}" title="Click to edit {{@$value->title}}" class="btn btn-xs btn-primary"><i class="fa fa-pencil" aria-hidden="true"></i></a>

                                  <a href="\admin\book\delete\{{$value->id}}" title="Click to delete {{@$value->title}}" onclick="return confirm('Are you sure?')" class="btn btn-xs btn-primary"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                </td>
                            </tr>
                        @endforeach
                    @else
