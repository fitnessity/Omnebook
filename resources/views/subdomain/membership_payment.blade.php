<div class="row">
	...
	{{-- <form id="payment-form" data-secret="{{$intent['client_secret']}}" style="padding: 16px;margin-bottom: 0px;" action="{{route('checkin_memberhsip-Pay')}}" method="POST" class="validation" data-cc-on-file="false"  data-stripe-publishable-key="{{ env('STRIPE_PKEY') }}" > --}}
		<form id="payment-form" data-secret="{{$intent['client_secret']}}" style="padding: 16px;margin-bottom: 0px;" method="POST" class="validation" data-cc-on-file="false"  data-stripe-publishable-key="{{ env('STRIPE_PKEY') }}" >
		@csrf
		...
		<input type="hidden" name="cartWidgetIds" id="cartWidgetIds" value="{{ json_encode($cartWidgetIds) }}">
	        ...
	        if(cardinfoRadio == null) {
	            ...
	            stripe.confirmSetup({
        	    	...
        	    	if (result.error) {
        	    		...
        	    		$.ajax({
							url: '{!! route('customers_refresh_payment_methods', ['business_id' => '__business_id__', 'customer_id' => '__customer_id__']) !!}'
							.replace('__business_id__', encodeURIComponent(businessId))
							.replace('__customer_id__', encodeURIComponent(customer_id)),
		                ...
		                $form.submit();*/
		                $.ajax({
							url:'{{ route("checkin_memberhsip-Pay") }}',
                            method: 'POST',
                            data: $form.serialize(),
