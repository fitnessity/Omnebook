...
<style>
.page-content {
  padding: calc(0px + 7.5rem) calc(1.5rem* .5) 60px calc(1.5rem* .5);
}
</style>
...
<script>
	...
	function openPopUp(scheduleId,sid,activityName,time,chk,catId){
		if ("{{Auth::check()}}") { 
			...
			$.ajax({
				url:'{{route("setSessionOfSchedule_sub")}}',
				type: 'POST',
				data:{
					_token: '{{csrf_token()}}',
				...
				success:function(data){
                    ...
                    window.location.href = '/login/' + '{{ $unique_code }}';
				}
			});
		}
	}
	...
	function timeBookingPopUP(scheduleId,sid,activityName,time,chk,catId) {
		...
		var membershipHtml = '';
		$.ajax({
			url:'{{route("chkOrderAvailable_sub")}}',
			type: 'POST',
			data:{
				_token: '{{csrf_token()}}',
				sid : sid,
