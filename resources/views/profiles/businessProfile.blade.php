			...
			<div class="tab-content">
                <div class="tab-pane" id="tabs-5" role="tabpanel">
                	<div class="loadMore">
                    	...
                    	@foreach($postsave as $posts)
                        <?php
						
							
							$page_post = PagePost::where('id',$posts->post_id)->first();
							$userData = User::where('id',$page_post->user_id)->first();
                        	
							 ?>
                        	<div class="central-meta item" id="listsavepostid<?php echo $page_post->id; ?>">
                            	<div class="user-post">
                                	
                                    
                                    <div class="post-meta">
                                    	<input type="text" name="abc" data-emojiable="true" data-emoji-input="image" class="removepost pb-30" value="{{$page_post['post_text']}}" disabled="">
                                        <?php 
										$userid = $page_post['user_id'];
										$count = count(explode("|",$page_post['images']));
										$countimg = $count-5;
                                        ...
                                        <figure>
											...
											$activethumblike='';
                                            ?>
                                            <ul class="like-dislike" id="ulike-dislike-saved<?php echo $page_post->id; ?>">
                                            	<?php $loginuser_like = PagePostLikes::where('post_id',$page_post['id'])->where('is_like',1)->where('user_id',$loggedinUser->id)->first(); ?>
                                                @if(!empty($loginuser_like))
													<?php $activethumblike='activethumblike'; ?>
												@endif
                                               <li><a id="savepost{{$page_post['id']}}" class="bg-purple savepost <?php echo $savedpost; ?>" href="javascript:void(0);" title="Save to Pin Post" postid="{{$page_post['id']}}" pageid="{{ request()->page_id }}" posttype="Saved">
												<i class="thumbtrack fas fa-thumbtack"></i></a>
												</li>
                                                <li><a class="thumbup thumblike <?php echo $activethumblike; ?>" href="javascript:void(0);" title="Like Post" id="like-thumb-saved<?php echo $page_post->id; ?>" postid="{{$page_post['id']}}" is_like="1" posttype="Saved" ><i class="fas fa-thumbs-up"></i></a></li>
                                                <li><a class="bg-red" href="javascript:void(0);" title="dislike Post"><i id="{{$page_post['id']}}" postid="{{$page_post['id']}}" is_like="0" class="thumpdown thumblike fas fa-thumbs-down"></i></i></a></li>
											</ul>
										</figure>
                                        <div class="we-video-info">
											<ul>
												<li>
													<span class="views" title="views">
														<i class="eyeview fas fa-eye"></i><ins>1.2k</ins>
													</span>
												</li>
												<li>
													<div class="likes heart" title="Like/Dislike">
														<i class="fas fa-heart"></i>
                                                        <span id="likecount-saved{{$page_post['id']}}">{{$page_posts_like}}</span>
                                                    </div>
												</li>
												<li>
													<span class="comment{{$page_post->id}}" title="Comments">
                                            ...
                                            <div class="users-thumb-list" id="users-thumb-list-saved<?php echo $page_post->id; ?>">
												<?php
													$page_posts_like = PagePostLikes::where('post_id',$page_post->id)->where('is_like',1)->count(); ?>
                                                    @if($page_posts_like>0)
														@if(!empty($loginuser_like))
														...
														@endif
                                                   		<?php 
                                                            ...
                                                            <span>
                                                            	<strong>
                                                                	@if(!empty($loginuser_like)) You @endif
																</strong>
																@if(!empty($seconduser_like))
																	<?php $secondusername = User::where('id',$seconduser_like->user_id)->first(); ?>,<b>{{$secondusername->username}}</b>
																@endif
