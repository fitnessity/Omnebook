...
<div id="messageDiv" style="display:none;"></div>
            ...
            <tbody>
            	@foreach ($customers as $ubd) 
                    <tr>
                        ...
                        <td>
							{{-- <button type="submit" class="btn btn-red" onclick="CancelFreeze({{encrypt($ubd->id)}})">Cancel Freeze</button> --}}
						    <button type="submit" class="btn btn-red" onclick="CancelFreeze({{ json_encode(encrypt($ubd->id)) }})">Cancel Freeze</button>
						</td>
					</tr>
                @endforeach
            </tbody>
	...
	@php $business_id = $company->id; @endphp
	<script>
		function CancelFreeze(bid) {
			$.ajax({
				url: "/business/{{$business_id}}/cancel_freeze",  // Corrected the quotes here
				type: "POST",
				data: {
					_token: '{{ csrf_token() }}', 
					booking_id: bid,
				...
				success: function(response) {
					$('#messageDiv').text(response.message).css('color', 'green').show(); // Show success message in green
					setTimeout(function() {
						location.reload();
					}, 3000);
				},
