							...
							<div class="col-md-3 col-xs-12 col-sm-6">
								<div class="date-activity-scheduler">
					                ...
					                <div class="activityselect3 special-date">
										<input type="text" name="date" class="form-control" autocomplete="off" value="{{$filterDate->format('m/d/Y')}}" data-behavior="on_change_submit datepicker">
									</div>
		              			</div>
							</div>
							<div class="col-md-6 col-xs-12 col-sm-12">
							                ...
							                <div class="row">
					    	                	<div class="col-md-1 col-xs-12 col-sm-4">
	    											<div class="timeline">
	    												...
	    												<span>{{date('h:i A', strtotime($schedule['shift_start']))}}</span>
	    												<span>{{date('h:i A', strtotime($schedule['shift_end']))}}</span>
	    											</div>
	    										</div>
						                        <div class="col-md-1 col-xs-12 col-sm-4">    
						                        	<a href="{{route('business.schedulers.checkin_details.index',['scheduler'=>$schedule->id, 'date' =>$filterDate->format('m/d/Y')])}}" class="scheduler-qty">
						                        		<label class="scheduler-titles">QTY: </label> 
						                        		<span> {{$schedule->spots_left($filterDate)}}/{{$schedule->spots_available}} </span>
						                        	</a>
						                        </div>
					    	                  	<div class="col-md-1 col-xs-12 col-sm-4 nopadding">
						    	                  	<div class="scheduled-activity-info">
						    	                  		<label class="scheduler-titles"> Duration: </label> <span>{{$schedule->get_duration()}} </span>
						    	                  	</div>
					    	                  	</div>
	    										<div class="col-md-3 col-xs-12 col-sm-4">
	    											<div class="scheduled-activity-info">
	    												...
	    												<span>
	    													{{$schedule->business_service->program_name}}<br/>
	    													@if($schedule->businessPriceDetailsAges()->exists())
						                        ...
						                        <div class="col-md-2 col-xs-12 col-sm-4">
						                         	<div class="scheduled-location">
						                         		<label class="scheduler-titles"> Location: </label> <span> {{$schedule->business_service->activity_location}}</span>
						                         	</div>
						                        </div>
				                  				<div class="col-md-2 col-xs-12 col-sm-4">
						                          	<div class="scheduled-location">
								                ...
								                <div class="col-md-1 col-xs-12 col-sm-12">
						                            ...
						                            <input type="hidden" name="serviceid" value="{{$schedule->business_service->id}}">
		                			...
		                			<div class="scheduler-info-box">
			                			<div class="row">
											...
											<div class="col-md-3 col-xs-12 col-sm-4">
												<div class="scheduled-activity-info">
													<label class="scheduler-titles"> Scheduled Activity: </label> 
													<span>{{$schedule->business_service->program_name}}
															<br/>
															@if($schedule->businessPriceDetailsAges()->exists())
	    														{{$schedule->businessPriceDetailsAges->category_title}}
	    													@endif
						                    ...
						                    <div class="col-md-2 col-xs-12 col-sm-4">
		                  					...
		                  					<div class="col-md-2 col-xs-12 col-sm-12">
					                            ...
					                            <input type="hidden" name="serviceid" value="{{$schedule->business_service->id}}">
					                            <button type="submit" class="btn-edit btn-sp">Edit</button>
					                            <button type="button" class="btn-edit" data-behavior="ajax_html_modal" data-url="{{route("business.schedulers.delete_modal", ['business_scheduler_id' => $schedule->id, 'date' => $filterDate->format('m/d/Y'), 'return_url' => url()->full()])}}">Cancel</button>
							                    	</div>
							                  	</form>
		                  					</div>
		                				</div>
					...
					<div class="row">
						...
						<div class="col-md-6 col-xs-12 col-sm-6">
							<div class="pre-next-btns pre-nxt-btn-space">
								<a class="btn-previous btn-sp" style="background: darkgray;" href="{{route('business.schedulers.index', array_merge(request()->query(), ['date' => $filterDate->subDay()->format('m/d/Y')]))}}"  disabled id="btn-pre"><i class="fas fa-caret-left preday-arrow" ></i>Previous Day</a>
								<a class="btn-previous" id="btn-next" href="{{route('business.schedulers.index', array_merge(request()->query(), ['date' => $filterDate->addDay()->format('m/d/Y')]))}}">Next Day <i class="fas fa-caret-right nextday-arrow"></i></a>
							</div>
						</div>
					</div>
				</div>	
