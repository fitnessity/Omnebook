<div class="col-md-12 col-sm-12 mb-12 {{$type}}" >
	...
	<div class="hstack gap-3 px-3 mx-n3">
		<select class="price-select-control fs-13 familypart form-control" name="participat[]" id="participats_1">
...
<script>
	...
	// 		};
	// 	$.post('{{ route("formparticipate") }}', data).done(function() {
...
<script>
    $('.familypart').change(function() {
        ...
        if(value == 'addparticipate'){
            ...
            $.post('{{ route("formparticipate") }}', data)
            .done(function(response) {
                $(".participaingdiv" + data.act).load(" .participaingdiv" + data.act + ">*");
                // alert('Participant successfully added/updated!'); // Display a success message
                console.log('Success:', response); // Log the success response
            })
            .fail(function(xhr, status, error) {
                // Error occurred
                alert('An error occurred: ' + error); // Display an error message
                console.error('Error:', xhr, status, error); // Log the error details
            })
            .always(function() {
                // This code runs regardless of success or failure
                console.log('AJAX request finished.'); // Log when the AJAX request is completed
            });
        }
    });
</script>
