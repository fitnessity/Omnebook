                     ...
                     <div class="row mb-3 printnone">
                     	<div class="col-12">
								...
								<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
									<div class="page-heading">
										<label>Money Owed Details & History</label>
									</div>
								</div><!--end col-->
							</div><!--end row-->
												...
												<div class="accordion custom-accordionwithicon accordion-border-box" id="accordionnesting">
													...
													<div class="accordion-item shadow">
														<h2 class="accordion-header" id="accordionnestingExampleAll">
															<button class="accordion-button collapsed uppercase" type="button" data-bs-toggle="collapse" data-bs-target="#accor_nestingExamplecollapseAll" aria-expanded="false" aria-controls="accor_nestingExamplecollapseAll" onclick="getData('All','')" id="Alldaysbtn">Missed Credit Card Payments </button>
														</h2>
...
<script>
	...
	function getData(type,limit){
		...
		$.ajax({
	  		type: "post",
         url: "{{route('business.money_owed.getMemberships')}}",
         data: {
         	endDate: endDate,
         	startDate: startDate,
         	type: type,
   ...
   function loadMoreRecords(type) {
   	let startDate = $('#startDate').val();
		let endDate = $('#endDate').val();
     	isLoading = true;
     	$.ajax({
         url: "{{route('business.money_owed.getMoreMemberships')}}",
         method: 'GET',
         data: { 
         	offset: offset,
         	endDate: endDate,
   ...
   flatpickr(".flatpickr-range", {
	...
	$(document).on('click', '[data-behavior~=on_change_submit]', function(e){
		...
		if(sdate && edate){
		 	e.preventDefault()
			getData('All');
			$('#Alldaysbtn').removeClass('collapsed');
			$('#onTodaydaysbtn, #FailedPaymentdaysbtn,#WhoOwnMoneydaysbtn').addClass('collapsed');
			...
			$('#accor_nestingExamplecollapseAll').addClass('show');
	...
	function exportData(){
      ...
      var filename =  '';
		...
		if(type != '' && type != 'print'){
			...
			var downloadUrl = '{{ route("business.money_owed.export") }}' +
	        '?endDate=' + endDate +
	        '&startDate=' + startDate +
	        '&type=' + type;

	    	if(type == 'excel'){
	    		filename = 'MoneyOwedDetail.xlsx';
	    	}else if(type == 'pdf'){
	    		filename = 'MoneyOwedDetail.pdf';
	    	}
