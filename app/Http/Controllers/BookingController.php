        ...
        if($request->type == 'current'){
            if(!empty($bookingstatus)){
                foreach($bookingstatus as $Bstatus){
                    if($serviceType== null || $serviceType == 'all'){
                        $booking_details = UserBookingDetail::where(['booking_id'=>$Bstatus->id,'user_id'=>$customer->id ]);
                        if($request->date != ''){
                            $booking_details->whereDate('bookedtime', '=', date('Y-m-d',strtotime($request->date)));
                        }
                        $booking_details = $booking_details->get();
                    }else{
                        $booking_details = UserBookingDetail::join('business_services', 'user_booking_details.sport', '=', 'business_services.id')->where(['business_services.service_type'=>$serviceType ,'user_booking_details.booking_id',$Bstatus->id,'user_booking_details.user_id',$customer->id ]);
                        if($request->date != ''){
            ...
            if(!empty($bookingstatus)){
                foreach($bookingstatus as $Bstatus){
                    ...
                    foreach($booking_details  as $details){
                        $Booking_checked_indetail = BookingCheckinDetails::where(['booking_detail_id'=>$details->id]);
                        if($request->date != ''){
                            $Booking_checked_indetail->whereDate('checkin_date', '=', date('Y-m-d',strtotime($request->date)));
                        } 

                        $Booking_checked_indetail = $Booking_checked_indetail->get();
            ...
            $BookingDetail = $this->bookings->tabFilterData($checkIndetail,$request->type,$request->serviceType,date('Y-m-d',strtotime($request->date)));
        }
                ...
                $html.='<div class="col-md-4 col-sm-6 ">
                            <div class="boxes_arts">
                                <div class="headboxes">
                                    ...
                                    if($request->type == 'current'){
                                        $html.='<div class="booking-active'; 
                                            if($book_details->pay_session >0 ){ 
                                                $html.='booking-active-color';
                                            }else {
                                                $html.='booking-inactive-color';
                                            }
                                        $html.='">';
                                        if($book_details->pay_session >0 ){
                                            $html.='Active'; 
                                    ...
                                    $html.='<div class="highlighted_box">Confirmed</div>
                                </div>
            ...
            if(!empty($bookingstatus)){
                foreach($bookingstatus as $Bstatus){
                    if($serviceType== null || $serviceType == 'all'){
                        ...
                        $booking_details = UserBookingDetail::join('business_services', 'user_booking_details.sport', '=', 'business_services.id')->where('business_services.service_type',$serviceType)->where('user_booking_details.booking_id',$Bstatus->id)->where('user_booking_details.user_id',$customer->id)->get();
                    }
