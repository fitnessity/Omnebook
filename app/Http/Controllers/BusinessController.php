...
class BusinessController extends Controller
{
    ...
    public function bookingchart(Request $request){
        ...
        if($request->type == 'revenue'){
            $data = array('in_person' =>  $in_person_cnt,'online'   =>  $online_cnt,);
        }else{
            $data = array('ptdata' => $ptdata,'clsdata' => $clsdata,'expdata' => $expdata,'evdata' => $evdata,'prdata' => $prdata, );
        }
    ...
    public function getscheduleactivity(Request $request){
        ...
        $business = Auth::user()->current_company;
        if($request->date == ''){ $date = date('Y-m-d'); }else{ $date =  date('Y-m-d' ,strtotime($request->date)); }
	...
	public function pagePost(Request $request) {
        ...
        if($request->hasFile('image_post')){  
            ...
            $data=array("post_text" => $request->post_text,"user_id" => $loggedinUser->id,"video" => $name,"page_id" => $page_id,);
			PagePost::create($data);
        }
            ...
            $data=array("post_text" => $request->post_text,"user_id" => $loggedinUser->id,"music" => $name,"page_id" => $page_id,);
           PagePost::create($data);
        }
			...
			$data=array("post_text" => $request->post_text,"user_id" => $loggedinUser->id,"images" => $imgname.'.png',"page_id" => $page_id,);
            PagePost::create($data);
		}
		else if($request->post_text != '' && !$request->hasFile('music_post') && !$request->hasFile('video') && !$request->hasFile('image_post') && !$request->selfieimg ){
			$data=array("post_text" => $request->post_text,"user_id" => $loggedinUser->id,"page_id" => $page_id,);
           PagePost::create($data);
		}
        return Redirect::back()->with('success', 'Post created succesfully!');
    }
	public function pagePostcomment($id,Request $request) { 
       $data=array("user_id" => Auth::user()->id,"post_id" => $id,"comment" => $request->comment,);
	...
	public function pageshowcomments($id,Request $request) { 
        ...
        foreach ($commentData as $comment) {
			...
			if($cmntUlike>0){ $commentLiked='commentLiked'; }
            $html .= '<li>
                    <div class="comet-avatar"><img src="/public/uploads/profile_pic/thumb/'.$username->profile_pic.'" alt=""></div>
                    <div class="we-comment">
                        <h5><a href="javascript:void(0);" title="">'.$username->firstname.' '.$username->lastname.'</a></h5>
                        <p>'.$comment->comment.'</p>
                        <div class="inline-itms">
	...
	public function commentLike($id,Request $request) {
		...
		$status='';
		if(!empty($like)){
			PagePostCommentsLike::find($like->id)->delete(); $status='unlike';
		}
		else
		{
			$data=array("user_id" => Auth::user()->id,"post_id" => $request->postId,"comment_id" => $id,);
	...
	public function likepost($id,Request $request) {
      ...
      if(!empty($like)){
            ...
            $like->is_like = $saved; $like->update(); 
            ...
            $saved='1';
            $data=array("user_id" => Auth::user()->id,"post_id" => $id,"is_like" => $request->is_like,);
    ...
    {
		$postid=$request->postid; $pageid=$request->pageid;
		...
		{
			$array = array("post_id" => $postid,"page_id" => $pageid,"user_id" => Auth::user()->id);
	...
	public function savepagecoverphoto(Request $request) {
        ...
        $this->validate($request, [ 'coverphoto' => 'required|dimensions:min_width=800,min_height=450', ]);
        $imageName = '';
		...
		$array = array(
			"page_id" => $pageid,"user_id" => Auth::user()->id,"attachment_name" => $imageName,"attachment_status" => '1',"cover_photo" => "1",
		);
	...
	public function editPageProPic(Request $request) {
        ...
        if (!$request->hasFile('profile_pic')) {
            $response = array(
                'type' => 'danger','msg' => 'No image found to upload',
            );
            return Response::json($response);
        }
        ...
        if (!$userObj->save()) {
            $response = array('type' => 'danger','msg' => 'Some error while updating profile picture.',);
            return Response::json($response);
        } else {
            ...
            $response = array(
                'type' => 'success','msg' => 'Profile picture updated succesfully!',
                'returndata' => array('profile_pic' => $image_path)
            );
	...
	public function pagePostupdate(Request $request) { 
		...
		if($request->hasFile('video')){
            $this->validate($request, ['video' => 'required|mimes:mp4,mov,ogg,qt | max:10000',]);
            $imagebanner = $request->file('video');        
            $name = $imagebanner->getClientOriginalName();        
            $imagebanner->move(public_path().DIRECTORY_SEPARATOR.'uploads'.DIRECTORY_SEPARATOR.'gallery'.DIRECTORY_SEPARATOR. $loggedinUser->id .DIRECTORY_SEPARATOR.'video'.DIRECTORY_SEPARATOR, $name);
            $videopost = $name;
		...
		if(!empty($company)) {
            ...
            if (isset($serviceData)) {
                foreach ($serviceData as $service) {
                    ...
                    if(!empty($company)) { $servicePrice[@$company['id']][] = $price; }
	...
	public function followPage(Request $request) {
		...
		$follow_id = 0;
		$followpro = PageLike::create([ 'pageid' => $pageid,'follower_id' => $userid, ]);
		if($followpro){
                    		...
                    		<div class="row">
                        		<div class="col-md-6 col-lg-6">
                            		<div class="bookinfo-title">
										...
										<div class="bookinfo"><h4>Booking Info</h4></div>
										<div class="booking-detail"><span>Friday, May 7th, 2021</span></div>
										<div class="booking-detail"><span>09:00 am - 10:00 am</span></div>
										<div class="booking-detail"><span>Service Type: </span> <span>'.@$act['service_type'].'</span></div>
										<div class="booking-detail"><span>Duration: </span> <span>1 hour</span></div>
										<div class="booking-detail"><span>Activity Location: </span> <span>'.@$act['activity_location'].'</span></div>
										<div class="booking-detail"><span>Spots Left: </span><span><?php echo $SpotsLeftdis; ?>/'.@$act['group_size'].'</span></div>
										<div class="booking-detail"><span>Service For: </span> <span>'.@$act['activity_for'].'</span></div>
										<div class="booking-detail"><span>Age: </span> <span>'.@$act['age_range'].'</span></div>
										<div class="booking-detail"><span>Language: </span> <span>-</span></div>
									</div>
								</div>
								<div class="col-md-6 col-lg-6">
                            		<div class="price-part">
										...
										<div id="bookajax'.@$act["id"].'">
											<p>Price Option: '.$pay_session1 .' Session</p><p>Participants: '.@$act['group_size'].'</p>
	...
	public function save_business_reviews(Request $request)
	{
		...
		{ 
			...
			if(empty($chk))
			{
				...
				$data=array(
				   	"rating"=>$rating,"title"=>$title,"review"=>$review,"images"=>$imgpost,"user_id" => Auth::user()->id,"page_id" => $page_id,"ip" => $ip,
				);
    ...
    public function add_business_customer(Request $request)
    {
        ...
        if($comdata != ''){
            ...
            if($comdata->state != ''){ $address .= $comdata->state.', '; }
            if($comdata->country != ''){ $address .= $comdata->country.', '; }
            if($comdata->zip_code != ''){ $address .= $comdata->zip_code; }
