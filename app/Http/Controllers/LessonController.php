...
class LessonController extends Controller {
    ...
    public function __construct(UserRepository $users, BookingRepository $bookings, Request $request, SportsRepository $sports) {
        $this->users = $users; $this->bookings = $bookings; $this->sports = $sports;
    }
    public function jsModallesson($modelName, $sportId = 0) {
        $s = Sports::where('id', $sportId)->first();
        if ($s) { $booking_option = $s->booking_option; } else { $booking_option = ''; }
        $sportsListRaw = $this->sports->getAlphabetsWiseSportsNames();
        $sportsList = array(); $totalSports = 0;
        if ($sportsListRaw) {
            foreach ($sportsListRaw as $key => $value) {
                foreach ($value as $key1 => $value1) { $totalSports+=(count($value1->child) + 1); $sportsList[] = $value1; }
            }
        }
    ...
    public function GetjobsSubmit($id) {
        if (isset($id) && !empty($id)) {
            ...
            $qutObj['jobid'] = $id; $qutObj['quote'] = $userQut;
        }
    ...
    public function filter($request, $filter) {
        ...
        if ($filter['professional_type'] != null) {
            $search = $filter['professional_type']; $q->orWhere('sport_activity', 'LIKE', '%'. $search . '%');
        }
        if ($filter['activity_location'] != null) {
            $company_ids = []; $search = $filter['activity_location'];
            $searchDatas->where(function($q) use ($search) {
                if(!in_array("any", $search)){
                    foreach ($search as $data) {
                        if(strpos($data,'_')!= ''){ $data = ucwords(str_replace('_',' ', $data)); }
                        else{ $data = ucwords($data); }
        ...
        if ($filter['location'] != null) {
            $company_ids = []; $search = $filter['location'];
            $searchDatas->where(function($q) use ($search) {
                $q->orWhere('exp_city', 'LIKE', '%'. $search . '%')->orWhere('exp_state', 'LIKE', '%'. $search . '%')->orWhere('exp_zip', 'LIKE', '%'. $search . '%');
            });
        }
        if ($filter['activity_type'] != null) {
            $company_ids = []; $search = $filter['activity_type'];
            $searchDatas->where(function($q) use ($search) {
                if(!in_array("any", $search)){
                    foreach ($search as $data) {
                        $data = ucwords($data);  $q->orWhere('activity_for', 'LIKE', '%'. $data . '%');
                    }
                }
            });
        }
        if ($filter['age_range'] != null) {
            $company_ids = []; $search = $filter['age_range'];
            $searchDatas->where(function($q) use ($search) {
                if(!in_array("any", $search)){
                    foreach ($search as $data) {
                        if(strpos($data,'_')!= ''){ $data = ucwords(str_replace('_',' ', $data)); }
        ...
        if ($filter['duration'] != null) {
            $company_ids = []; $search = $filter['duration'];
            $searchDatas->where(function($q) use ($search) {
                foreach ($search as $data) {
        ...
        if ($filter['cnumber_people'] != null) {
            $company_ids = []; $search = $filter['cnumber_people'];
            $searchDatas->where(function($q) use ($search) {
                $q->orWhere('group_size', '=', $search);
            });
        }
        if ($filter['difficulty_level'] != null) {
            $company_ids = []; $search = $filter['difficulty_level'];
            $searchDatas->where(function($q) use ($search) {
                if(!in_array("any", $search)){
                    foreach ($search as $data) {
                        $data = ucwords($data); $q->orWhere('difficult_level', 'LIKE', '%'. $data . '%');
                    }
                }
            });
        }
        ...
        if ($filter['personality_habit'] != null) {
            $company_ids = []; $search = $filter['personality_habit'];
            $searchDatas->where(function($q) use ($search) {
                foreach ($search as $data) {
                    if(strpos($data, '_')!= ''){ $data = ucwords(str_replace('_',' ', $data)); }
                    else{ $data = ucwords($data); }
        ...
        if ($filter['activity_Member'] != null) {
            $company_ids = []; $search = $filter['activity_Member'];
        } 
    ...
    public function getDirecthire(Request $request) {
        if (isset($request->selected_sport)) { $request->session()->put('selected_sport', $request->selected_sport); } 
        else { $request->session()->forget('selected_sport'); }
        if (isset($request->level_of_experience)) {  $request->session()->put('level_of_experience', $request->level_of_experience); }
        else { $request->session()->forget('level_of_experience'); }
        if (isset($request->who_is_training)) {  $request->session()->put('who_is_training', $request->who_is_training);  }
        else { $request->session()->forget('who_is_training'); }
        if (isset($request->gender)) {  $request->session()->put('gender', $request->gender); }
        else { $request->session()->forget('gender'); }
        if (isset($request->personality)) {  $request->session()->put('personality', $request->personality); }
        else { $request->session()->forget('personality'); }
        if (isset($request->availability_days)) { $request->session()->put('availability_days', $request->availability_days); }
        else { $request->session()->forget('availability_days'); }
    ...
    public function getInstanthireSearch(Request $request) {
        $newSearch = array(); $newSearchActivity = array();
        $profilename = array();
        if ($request->has('keyword') && $request->input('keyword')!='') {
            ...
            $searchDatasActivity = BusinessServices::where('instant_booking', 1)->where('is_active', 1)->where('sport_activity', 'LIKE', $keyword . '%')->groupBy('id')->get();
    ...
    public function getInstanthire(Request $request) {
        ...
        if (isset($request->selected_location)) {
            ...
            $request->session()->forget('selected_location'); $request->session()->forget('miles_radius_filter');
        }
        ...
        if($request->activity_Member  != null) {
            ...
            if(!empty($search) ||$search[0]!= ''){
                $searchData->join('business_price_details', 'business_services.id', '=', 'business_price_details.serviceid')->select('business_services.*','business_price_details.membership_type')->groupby('business_services.id')->where(function($q) use ($search) {
                    foreach ($search as $data) {
                        $data = ucwords($data);  $q->orWhere('membership_type', 'LIKE', '%'. $data . '%');
                    }
                });
            }
        }
        ...
        if($request->activity_location != null  ){
            ...
            if(!empty($search) ||$search[0]!= ''){
                $searchData->where(function($q) use ($search) {
                    if(!in_array("any", $search)){
                        foreach ($search as $data) {
                            $data = ucwords($data); $q->orwhereRaw('FIND_IN_SET("'.$data.'",activity_location)');
                        }
                    }
                });
            }
        ...
        if($request->service_type_two != null ) {
            ...
            if(!empty($search)){
                $searchData->where(function($q) use ($search) {
                    foreach ($search as $data) { $q->orWhere('select_service_type', 'LIKE', '%'. $data . '%'); }
                });
            }
        }
        ...
        foreach($companyData as $company) {
            foreach($company as $value) {
                ...
                foreach ($locations as $key2 => $value2) {
                    if (($value2[1] == $value['latitude']) && ($value2[2] == $value['longitude'])) { $found = $found + 1; }
                }
                if ($found != 0) {
    ...
    public function postSaveDirecthireRequest(Request $request) {
        $loggedinUser = Auth::user(); $data = array(); $who = [];
    ...
    public function Getjobs($id) {
        if (Auth::check()) {
            if (isset($id) && is_numeric($id)) {
                ...
                if ($data) { return view::make('jobpost.jobs')->with('data', $data); }
            } else {
    ...
    public function viewBooking($id) {
        if (isset($id) && is_numeric($id)) {
            ...
            if (Auth::user()->role == "business") { $view = 'jobpost.professional.viewbooking'; }
            else {
            ...
            $followdetail = User::where('id', $data['jobsObj']['user_id'])->select('id')
                    ->with(['follows' => function ($q) use($fuser_id) {
                            $q->where('user_id', '=', Auth::User()->id); $q->where('follower_id', '=', $fuser_id);
                        }])
                    ->with(['favourites' => function ($q) use($fuser_id) {
                            $q->where('user_id', '=', Auth::User()->id); $q->where('favourite_user_id', '=', $fuser_id);
                        }])
                    ->first();
            return view($view, [
                'data' => $data,
    ...
    public function PostQuote($booking_id) {
        ...
        $bookingObj['jobpostquestions'] = $jobsObj;
        $data = [ 'jobid' => $booking_id, 'jobsObj' => $bookingObj ];
        return view('jobpost.professional.postquote', [
            'data' => $data, 'userQuote' => $userQuote, 'totalAndMaxQuotesForBooking' => $totalAndMaxQuotesForBooking,
            'sportsList' => $sportsList, 'pageTitle' => "MY BOOKINGS"
        ]);
    }
    public function SavePostQuote(Request $request) {
        ...
        if (!isset($request->quote_id) && $request->quote_id == 0) {
            ...
            if (@$totalAndMaxQuotesForBooking['total_quotes'] >= @$totalAndMaxQuotesForBooking['max_quotes']) {
                $response = array( 'type' => 'danger', 'msg' => 'Max quote limit reached for this booking.', );
                return Response::json($response);
            }
        }
        $data = array();
        $data['id'] = $request->quote_id;  $data['user_id'] = Auth::User()->id;  $data['booking_id'] = $request->booking_id;
        $data['quote_price'] = $request->quote_price;  $data['rate_type'] = $request->rate_type;  $data['quote'] = $request->quote_desc;
    ...
    public function GetUserQuoteList() {
        ...
        $sportsList = $this->sports->getAllSportsNames(1);
        return view($view, [
            'joblist' => $joblist, 'sportsList' => $sportsList, 'pageTitle' => "MY QUOTES"
        ]);
    }
    /* sam code */
    ...
    public function savetime(Request $r) {
        if (empty($r->hours)) { return response()->json(['status' => 0, 'msg' => "Please choose time slot"]); }
    ...
    public function getactivity(Request $request, $userid, $ser_id) {
        ...
        if (!empty($s)) {
            ...
            foreach ($s as $op) {
                $op1 = str_replace('_', ' ', strtoupper($op));  $data .= '<option value="' . $op . '">' . $op1 . '</option>';
            }
        } else {
    ...
    public function updatecart(Request $r) {
        $cart = array(
            'price' => str_replace('$', '', $r->price), 'numberofpersons' => $r->numberofpersons,
            'service_choice' => $r->selectcatagory, 'salestaxpercentage' => $r->salestaxpercentage,
    ...
    public function cartpaymentinstant(Request $request) {
        if($request->session()->has('checkoutsession')){ $request->session()->forget('checkoutsession'); }
        $cart = [];
        $cardInfo = [];
        if(Auth::user()){
        ...
        $cart = $request->session()->get('cart_item') ? $request->session()->get('cart_item') : [];
        return view('jobpost.cart-payment-instant',[ 'cart' => $cart, 'cardInfo' => $cardInfo, ]);
    }
    public function payment($token) {
        $payment = new StripePay;
        $paydata['email'] = Auth::user()->email; $paydata['amount'] = ''; $paydata['token'] = $token;
        ...
        $amount = $stripeResponse["amount"] / 100; $param_type = 'ssdssss';
        $param_value_array = array(
            $amount, $stripeResponse["currency"], $stripeResponse["balance_transaction"], $stripeResponse["status"], json_encode($stripeResponse)
        );
        $paymentdata = array(
            'email' => Auth::user()->email, 'item_number' => $paydata['nameofitems'], 'amount' => $amount,
            'currency_code' => $stripeResponse["currency"], 'txn_id' => $stripeResponse["balance_transaction"],
            'payment_status' => $stripeResponse["status"], 'payment_response' => json_encode($stripeResponse)
        );
    ...
    public function service_fav(Request $request) {
        $ser_id = $request->ser_id; $loggedId = Auth::user()->id; $status='';
        $favData = BusinessServicesFavorite::where('user_id',$loggedId)->where('service_id',$ser_id)->first();
        if(!empty($favData)){ BusinessServicesFavorite::find($favData->id)->delete(); $status='unlike'; }
        else
        {
            $data=array( "user_id" => Auth::user()->id, "service_id" => $ser_id, );  BusinessServicesFavorite::create($data);  $status='like';
        }
    ...
    public function save_business_service_reviews(Request $request)
    {
       $validatedData = $request->validate([
            'name' => 'required|string|max:255', 'title' => 'required', 'message' => 'required'
        ], [
            'name.required' => 'Enter name.', 'title.required' => 'Enter title.', 'message.required' => 'Enter message.',
        ]);
        ...
        if($review){
            ...
            if ($request->hasFile('images')) {
                foreach ($request->images as $file) {
                    $imagestore = $file->store('review'); $imgpost .= $imagestore . ',';
                }
            }
            $imgpost = rtrim($imgpost, ',');
            $data= [
                "name"=>$request->name, "title"=>$request->title,
                "message"=>$request->message, "images"=>$imgpost,
                "user_id" => Auth::user()->id, "service_id" => $request->sid,
                "ip" => $request->ip(),'cleanliness' => $request->cleanliness,
                'accuracy' => $request->accuracy, 'checkin' => $request->checkin,
                'communication' => $request->communication, 'customer_service' => $request->customer_service,
                'location' => $request->location, 'value' => $request->value, 'rating' => 0
            ];
    ...
    public function viewActreview(Request $request) {
        $aid = $request->aid; $data='';
        $reviews_count = BusinessServiceReview::where('service_id', $aid)->count();
        $reviews_sum = BusinessServiceReview::where('service_id', $aid)->sum('rating'); $reviews_avg=0;
        if($reviews_count>0)
        { $reviews_avg = round($reviews_sum/$reviews_count,2); }
        $reviews = BusinessServiceReview::where('service_id', $aid)->get();
        $reviews_people = BusinessServiceReview::where('service_id',$aid)->orderBy('id','desc')->limit(6)->get(); 
                    ...
                    <div class="col-md-8"> 
                        <h3 class="subtitle"> 
                            <div class="row">
                                ...
                                <div class="col-md-9"><p> <a class="activered font-bold"> By Everyone  </a><a class="font-bold"> | By People I know </a> </p></div>
                            </div>
                        </h3>
                        <div class="service-review-desc">
                            <p> '.$reviews_count.' Reviews </p> <div class="rattxt activered"><i class="fa fa-star" aria-hidden="true"></i> '.$reviews_avg.' </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                            ...
                            foreach($reviews as $review) {
                                 ...
                                 $data .='<div class="ser-rev-user">
                                            <div class="row">
                                                <div class="col-md-2">';
                                                if(File::exists(public_path("/uploads/profile_pic/thumb/".@$userinfo->profile_pic)))
                                                { $data .='<img class="rev-img" src="'.$pic.'" alt="'.@$userinfo->firstname.' '.@$userinfo->lastname.'">'; }
                                                else
                                                {
                                                    $pf=substr(@$userinfo->firstname, 0, 1).substr(@$userinfo->lastname, 0, 1);
                                                ...
                                                $data .='</div>
                                                        <div class="col-md-10">
                                                            ...
                                                            <div class="rattxt activered"><i class="fa fa-star" aria-hidden="true"></i> '.$review->rating.' </div> </h4> <p class="rev-time">'.date('d M-Y',strtotime($review->created_at)).'</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="rev-dt">
    ...
    public function submitreview(){ return view('jobpost.submit_review'); }
    public function createTest(Request $request){ return view('profiles.createTest'); }
    public function getServiceData(Request $request) { echo 'success'; exit; }
    public function act_detail_filter_business_pages(Request $request){
        ...
        if( !empty($actdate) )
        {
            $dt = date('Y-m-d',strtotime($actdate) ); $enddt = date('Y-m-d',strtotime($actdate));
            ...
            foreach ($activity as  $act) {
                ...
                if ($act['profile_pic']!="") {
                    if(str_contains($act['profile_pic'], ',')){
                        ...
                        if( $pic_image[0] == ''){ $p_image  = $pic_image[1]; }
                        else{ $p_image  = $pic_image[0]; }
                    }else{
                ...
                if(!empty($pricearr)){  $price_all = min($pricearr); }
                $bookscheduler=''; $time='';
                                        ...
                                        <div class="find-activity">
                                            <div class="row">
                                                <div class="col-md-4 col-sm-4 col-xs-12 business-right-panel">
                                                    <div class="img-modal-left-bpage"><img src="'.$profilePic.'" ></div>
                                                </div>
                                                <div class="col-md-8 col-sm-8 col-xs-12 activity-data">
                                                    <div class="activity-inner-data"><i class="fas fa-star"></i><span> '.$reviews_avg.' ('.$reviews_count.')</span></div>';
                                                    if($time != ''){
                                                        $actbox .= '<div class="activity-hours time-hours"><span>'.$time.'</span></div>';
                                                    }
                                                    $actbox .= '<div class="activity-city text-left-page"><span>'.$companycity.', '.$companycountry.'</span>';
                                                    if(Auth::check()){
                                                        ...
                                                        if( !empty($favData) ){ $actbox .= '<i class="fas fa-heart"></i>'; }
                                                        else{ $actbox .= '<i class="far fa-heart"></i>'; } 
                                                        $actbox .= '</a></div> '; 
                                                    }else{
                                                    ...
                                                    $actbox .= '</div>
                                                        ...
                                                        if($price_all != ''){
                                                            $actbox .= '<div><span class="activity-time">From $'.$price_all.'/Person</span></div>';
                                                        }
                                                $actbox .= '</div>
                                                </div>
                                            </div>
                                    ...
                                    <div class="find-activity">
                                        <div class="row">
                                            <div class="col-md-4 col-sm-4 col-xs-12">
                                                <div class="img-modal-left"><img src="'.$profilePic.'" ></div>
                                            </div>
                                            <div class="col-md-8 col-sm-8 col-xs-12 activity-data">
                                                <div class="activity-inner-data"><i class="fas fa-star"></i><span> '.$reviews_avg.' ('.$reviews_count.') </span></div>';
                                                if($time != ''){
                                                    $actbox .= '<div class="activity-hours"><span>'.$time.'</span></div>';
                                                }
                                                $actbox .= '<div class="activity-city"><span>'.$companycity.', '.$companycountry.'</span>';
                                                if(Auth::check()){
                                                    $loggedId = Auth::user()->id;
                                                    $favData = BusinessServicesFavorite::where('user_id',$loggedId)->where('service_id',$act['id'])->first();
                                                    $actbox .= '<div class="serv_fav1" ser_id="'.$act["id"].'">
    ...
    public function instant_hire_search_filter(Request $request) {
     /* print_r($request->all());exit();*/
      ...
      if($request->programservices  != null) {
        ...
        $searchDatas->where(function($q) use ($search) {
          foreach ($search as $data) {
            $data = ucwords($data); $q->orWhere('sport_activity', 'LIKE', '%'. $data . '%');
          }
        });
      }
      if($request->service_type != null) {
        $search = $request->service_type;
        $searchDatas->where(function($q) use ($search) {
          foreach ($search as $data) { $q->orWhere('service_type', 'LIKE', '%'. $data . '%'); }
        });
      } 
      if($request->service_type_two != null) {
        $search = $request->service_type_two;
      ...
      if ($request->activity_for != null) {
        ...
        $result = $searchDatas->get()->toArray();  $page = $request->page ? $request->page : 1;
        $perPage = $request->page_size ? $request->page_size : 10;  $offset = ($page * $perPage) - $perPage;
        $resultnew = new LengthAwarePaginator(
        array_slice($result, $offset, $perPage, true), count($result), $perPage, $page, ['path' => $request->url(), 'query' => $request->query()]
        );
      ...
      if (!empty($filter_serchdata) && count($filter_serchdata)>0) { 
        ...
        foreach ($filter_serchdata as  $act) {
          ...
          $companyid = $company->id; $companyname = $company->dba_business_name;  $companycity = $company->city;
          $companycountry = $company->country; $companylogo = $company->logo; $companylat = $company->latitude; $companylon = $company->longitude;
          $redlink = str_replace(" ","-",$companyname)."/".$act['cid'];
          $service_type='';
          if($act['service_type'] !=''){
            if( $act['service_type']=='individual' ) $service_type = 'Personal Training'; 
          ...
          if(@$bookscheduler[0]['set_duration']!=''){
            $tm=explode(' ',$bookscheduler[0]['set_duration']);
            $hr=''; $min=''; $sec='';
            if($tm[0]!=0){ $hr=$tm[0].'hr. '; } if($tm[2]!=0){ $min=$tm[2].'min. '; }
            if($tm[4]!=0){ $sec=$tm[4].'sec.'; } if($hr!='' || $min!='' || $sec!=''){ $time =  $hr.$min.$sec; } 
          }
          $pricearr = []; $price_all = '';
          $price_allarray = BusinessPriceDetails::where('serviceid', $act['id'])->get();
          if(!empty($price_allarray)){
            foreach ($price_allarray as $key => $value) { $pricearr[] = $value->pay_price; }
          }
          if(!empty($pricearr)){ $price_all = min($pricearr); }
          $actbox .= '
              ...
              $favData = BusinessServicesFavorite::where('user_id',$loggedId)->where('service_id',$act['id'])->first(); 
                $actbox .='<div class="kickboxing-topimg-content" ser_id="'.$act['id'].'" >
                      ...
                      <div class="serv_fav1" ser_id="'.$act['id'].'">
                        <a class="fav-fun-2" id="serfav'.$act['id'].'">';
                        if( !empty($favData)){ $actbox .='<i class="fas fa-heart"></i>'; }else{ $actbox .='<i class="far fa-heart"></i></a>'; }
                        $actbox .='</div>';
                        if($price_all != ''){ $actbox .='<span>From $'.$price_all.'/Person</span>'; }
                        $actbox .='</div>';
                        }else { $actbox .='<div class="kickboxing-topimg-content">
                        <img src="'.$profilePic.'" class="productImg">
            ...
            $actbox .='<div class="bottom-content">
                ...
                <div class="activity-information">
                  ...
                  $actbox .='target="_blank">'.$act["program_name"].'</a></span><p>'.$service_type.' | '.$act["sport_activity"] .'</p>
                </div><hr>
    ...
    public function addToCart(Request $request) {
        ...
        if($request->chk == 'activity_purchase'){
           ...
           return 'Membership added successfully.';
            ...
            if($msg == ''){ $msg = config('app.url').'/success-cart/'.$priceid; }
        return $msg;
        }    
    }
    ...
    public function removeToCart(Request $request) {
        ...
        if(!empty($cart_item["cart_item"])) {
            foreach($cart_item["cart_item"] as $k => $v) {
                if($_GET["priceid"] == $v['priceid']) { unset($cart_item["cart_item"][$k]); }
            }
        }
        if (isset($cart_item)) { $request->session()->put('cart_item', $cart_item); } else { $request->session()->forget('cart_item'); }
        if($request->chk == 'purchase'){
            return redirect()->route('business.orders.create', ['business_id'=>Auth::user()->cid,'cus_id' => $request->pageid]);
        }else{  return redirect('/carts'); }
    }
    public function emptyCart(Request $request) {
        $request->session()->forget('cart_item'); return redirect('/payments/card'); 
    } 
}