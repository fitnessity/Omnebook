...
class ActivityController extends Controller {
	...
	public function next_8_hours(Request $request){
		...
		foreach ($services as $service) {
			 $priceDetails = BusinessPriceDetailsAges::where('serviceid', $service->id)->where('class_type', $service->service_type)->where('stype','1')->get();
	...
	public function index(Request $request,$filtervalue = null)
	{
		if($filtervalue == 'classes' || $filtervalue == 'personal_trainer' || $filtervalue == 'experience' || $filtervalue == 'all' || $filtervalue == 'thismonth' || $filtervalue == 'most_popular' || $filtervalue == 'trainers_coaches' || $filtervalue == 'ways_to_workout'|| $filtervalue == 'active_wth_fun_things_to_do' || $filtervalue == 'events_in_your_area' )
		{
			...
			if($filtervalue){
				...
				}else { $request->session()->forget('service_type'); }
				...
				}else { $request->session()->forget('program_type'); }
				...
				}else { $request->session()->forget('service_type_two'); }
    ...
    public function getCompareProfessionalDetailInstant($id) {
        ...
        foreach ($profiledetail as $profile) {
            ...
            if (isset($profile->ProfessionalDetail->availability) && $profile->ProfessionalDetail->availability!== "") {
                ...
                else{ $checkAvailability = $profile->ProfessionalDetail->availability; }
        ...
        $arrItems = array(); $setArray = array();
        if(count($data)>0){
            foreach($data as $k=>$val){
                $getId = explode('_',$k); $indexid = $getId[1]; $offer='';
    ...
    public function act_detail_filter(Request $request){
        ...
        {
            $searchData->join('business_price_details', 'business_services.id', '=', 'business_price_details.serviceid')->select('business_services.*','business_price_details.membership_type')->Where('membership_type', $actfilmtype);
        }
    ...
    public function getmodelbody(Request $request){
        ...
        $totalquantity = 0 ;  $timechkfrommodel = 1;
            ...
            $stactbox.='
                <div class="col-md-12 col-sm-12 col-xs-12">
					<div class="row">
						<div class="col-md-8 col-sm-8 col-xs-6">
							<div class="counter-titles">
								<p class="counter-age-heading">Children</p><p>Ages 2-12</p>
							</div>
						</div>
						<div class="col-md-4 col-sm-4 col-xs-6">
							<div class="qty mt-5 counter-txt">
            ...
            $stactbox.='
                <div class="col-md-12 col-sm-12 col-xs-12">
					<div class="row">
						<div class="col-md-8 col-sm-8 col-xs-6">
							<div class="counter-titles">
								<p class="counter-age-heading">Infants</p><p>Under 2</p>
							</div>
						</div>
						<div class="col-md-4 col-sm-4 col-xs-6">
							<div class="qty mt-5 counter-txt">
        ...
        $bookdata .='<div class="personcategory"> <span>Adults x 0 </span> <span>Kids x 0</span> <span>Infants x 0</span> </div>';
        $bookdata .=' <div class="cartstotal mt-20"><label>Total </label><span id="totalprice">$0 USD</span></div>';
        echo $stactbox.'~~'.$bookdata.'^^'.$timechkfrommodel;
    }
    public function pricecategory(Request $request){
        $actid = $request->actid; $catid = $request->catid; $sid = $request->sid;
        ...
        if (!empty($price)) { 
            ...
            if($dayval == 'Saturday' || $dayval == 'Sunday'){
                ...
                if (!empty(@$price)) {
                    foreach ($price as  $pr) {
                        $priceid =$pr['id'];
                        $stactbox .='<option value="'.$pr['id'].'">'.$pr['price_title'].'</option>'; $i++;
                    }
                }
            }else{
                if(!empty(@$price))
            ...
            foreach($BusinessActivityScheduler->get() as $bdt){
                ...
                $timedata12 .='<div class="col-md-6">
                    <div class="donate-now">
                        ...
                        if( $i==1){
                        	if($SpotsLeftdis != 0) { $timedata12 .='checked'; $i++; }
                        }
                        $timedata12 .='/>
    ...
    public function postRegistration_as_guest(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            if (!$this->users->validateUser($postArr['username'])) {
                $response = array(
                    'type' => 'danger', 'msg' => 'User name already exists. Please select different Name',
                );
                return Response::json($response);
            };
            if (count($postArr) > 0) {
                ...
                $customer = \Stripe\Customer::create([
                        'name' => $cus_name, 'email'=> $postArr['email'],
                ]);
                ...
                if ($userObj) {
	                ...
	                $response = array(
	                    'type' => 'success', 'msg' => 'Thank you for registering with Fitnessity. Please verify your email address.',
	                );
	                Auth::login($userObj);
	                Auth::loginUsingId($userObj->id, true);
	                return Response::json($response);
	            }else {
                    $response = array( 'type' => 'danger', 'msg' => 'Some wrror occured while registering. Please try again later.', );
                    return Response::json($response);
                }
            } else {
                $response = array( 'type' => 'danger', 'msg' => 'Invalid email or password', );
                return Response::json($response);
            }
        }
    }
