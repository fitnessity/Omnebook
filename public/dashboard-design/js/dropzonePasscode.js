    ...
    var dropzonePreviewNode = document.querySelector("#dropzone-preview-list-passcode"),
        previewTemplate = (dropzonePreviewNode.itemid = "", dropzonePreviewNode.parentNode.innerHTML),
        dropzone = (dropzonePreviewNode.parentNode.removeChild(dropzonePreviewNode), new Dropzone(".dropzone-passcode", {
            url: "https://httpbin.org/post",
            method: "post",
              maxFiles: 1, 
            previewTemplate: previewTemplate,
            previewsContainer: "#dropzone-preview-passcode",
            paramName: "passcode",
            success: function(file, response) {
                // Append the hidden input element with the dataURL and name attribute set to 'cover[]'
                $('form').append('<input type="hidden" name="passcode" value="' + file.dataURL  + '">');
            ...
            init: function() {
                ...
                this.on("addedfile", function(file) {
                    var existingFile =  $('#dropzone-preview-passcode').find('li').length; 
                    if (existingFile > 1) {
                        myDropzone.removeFile(file);
