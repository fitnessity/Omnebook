...
export class RowTool{
    ...
    constructor(builder) {
        ...
        let htmlbutton = '';
        if(builder.opts.rowHtmlEditor) htmlbutton = `<button title="${util.out('HTML')}" class="row-html">
        ...
        if(!rowMore){
            let html = `<div class="is-pop rowmore" style="z-index:10002;">
                <div style="display:flex;flex-flow:wrap;padding-top:3px;">
                    <button title="${util.out('Move Up')}" class="row-up"><span><svg class="is-icon-flex" style="width:15px;height:15px;"><use xlink:href="#ion-ios-arrow-thin-up"></use></svg></span>${util.out('Move Up')}</button>
                    <button title="${util.out('Move Down')}" class="row-down"><span><svg class="is-icon-flex" style="width:15px;height:15px;"><use xlink:href="#ion-ios-arrow-thin-down"></use></svg></span>${util.out('Move Down')}</button>
                    <button title="${util.out('Duplicate')}" class="row-duplicate"><span><svg class="is-icon-flex" style="width:12px;height:12px;"><use xlink:href="#ion-ios-photos-outline"></use></svg></span>${util.out('Duplicate')}</button>
                    ${htmlbutton}
                </div>
            </div>`;
    ...
    render(row) {
        ...
        if(!rowtool){
            ...
            let html = `<div class="is-tool is-row-tool">
                ...
                <button title="${util.out('More')}" class="row-more"><svg class="is-icon-flex"><use xlink:href="#ion-more"></use></svg></button>
                <button title="${util.out('Grid Editor')}" class="row-grideditor"><svg class="is-icon-flex"><use xlink:href="#ion-grid"></use></svg></button>
                <button title="${util.out('Delete')}" class="row-remove"><svg class="is-icon-flex"><use xlink:href="#ion-ios-close-empty"></use></svg></button>
            </div>`;
