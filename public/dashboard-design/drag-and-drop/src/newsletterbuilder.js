...
class NewsletterBuilder {
    constructor(opts = {}){
        ...
        this.loadScript(scriptUrl).then(() => {
            ...
            for (i = 0; i < this.opts.plugins.length; i++) {
                ...
                this.loadScript(pluginfile).then(() => {
                    numOfPlugins++;
                    if(numOfPlugins==this.opts.plugins.length){
        ...
        document.addEventListener('keydown', (e) => {
            if (e.which === 90 && (event.ctrlKey || event.metaKey)) {//CTRL-Z
    ...
    // Convenience constructor, so that the plugin can be called directly using: NewsletterBuilder.run({ ... });
    static run(opts = {}) {
        return new NewsletterBuilder(opts);
    }
    ...
    handleCellPaste(col) {
        ...
        const html = '<div style="position:absolute;z-index:-1000;top:' + tmptop + 'px;left:-1000px;width:1px;height:1px;overflow:auto;" name="idContentWord" id="idContentWord" contenteditable="true"></div>';
        dom.appendHtml(this.builderStuff, html);
        ...
        setTimeout(()=> {
            ...
            try {
                ...
                if (comCon && comCon.parentNode) {
                    try { comCon.parentNode.normalize(); } catch (e) {1;}
                }
    ...
    currentScriptPath() {
        ...
        for (var i = 0; i < scriptElements.length; i++) {
            ...
            filename = 'newsletterbuilder.js';
            if (source.indexOf(filename) > -1) {
                location = source.substring(0, source.indexOf(filename));
            }
            filename = 'newsletterbuilder.min.js';
            if (source.indexOf(filename) > -1) {
                location = source.substring(0, source.indexOf(filename));
            }
        }
...
(function(window){
    ...
    var $ = window.jQuery;
    if($) $.newsletterbuilder = function (options) {
        return new NewsletterBuilder(options);
    };
